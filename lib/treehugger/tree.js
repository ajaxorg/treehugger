...
function inRange(p, pos) {
    if(p && p.startLine <= pos.line && p.endLine >= pos.line)
        if(p.startLine === pos.line || p.endLine === pos.line)
            return p.startCol <= pos.col  && p.endCol >= pos.col;
        else
            return true;
}
...
Node.prototype.findNode = function(pos) {
    var p = this.getPos();
    console.log("Node findNode", p, pos);
    if(inRange(p, pos)) {
        return this;
    } else {
        return null;
...
ConsNode.prototype.getPos = function() {
    var pos = {startLine : 9999999999,
               startCol  : 9999999999,
               endLine   : 0,
               endCol    : 0};
    if (this.getAnnotation("pos")) {
        var nodePos = this.getAnnotation("pos");
        pos = {startLine : nodePos.startLine,
               startCol  : nodePos.startCol,
               endLine   : nodePos.endLine,
               endCol    : nodePos.endCol};
    }
    for (var i = 0; i < this.length; i++) {
        var p = this[i].getPos();
        if (p) {
            pos.startLine = Math.min(pos.startLine, p.startLine);
            pos.startCol  = Math.min(pos.startCol,  p.startCol);
            pos.endLine   = Math.max(pos.endLine,   p.endLine);
            pos.endCol    = Math.max(pos.endCol,    p.endCol);
        }
    }
    return pos;
};
...
ConsNode.prototype.findNode = function(pos) {
    var p = this.getPos();
    console.log("ConsNode findNode", this, p, pos);

    if(inRange(p, pos)) {
        for(var i = 0; i < this.length; i++) {
            var p2 = this[i].getPos();
            console.log("Child pos:", p2);
            if(inRange(p2, pos)) {
