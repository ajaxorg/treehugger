...
exports.transform = function(n) {
    ...
    switch(nodeName) {
        ...
        case "assign":
            ...
            break;
        case "dot":
            resultNode = tree.cons("FieldAccess", [transform(n[1]), tree.string(n[2])]);
            break;
        case "name":
    ...
    if(n[0].start) {
        var extraWidth = 0;
        if(n[0].start.col === n[0].end.col) {
            // Correct
            extraWidth = (""+n[1]).toString().length;
        }
        resultNode.setAnnotation("pos", {startLine: n[0].start.line, startCol: n[0].start.col,
                                         endLine: n[0].end.line, endCol: n[0].end.col + extraWidth});
    }
    return resultNode;
};
