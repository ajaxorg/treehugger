...
module.exports = {
    ...
    "test error recovery" : function() {
        ...
        // for in
        assert.equal(parser.parse("for(var p in something.) bla()").toString(), '[For(VarDecls([VarDecl("p")]),PropAccess(Var("something"),""),Call(Var("bla"),[]))]');
        assert.equal(parser.parse("for(var p in something.) bla()").toString(), '[For(VarDecls([VarDecl("p")]),PropAccess(Var("something"),""),Call(Var("bla"),[]))]');
        ...
        assert.equal(parser.parse("(function() { hello()").toString(), '[Function(\"null\",[],[Call(Var("hello"),[])])]');
        assert.equal(parser.parse("(function() { hello.").toString(), '[Function(\"null\",[],[PropAccess(Var("hello"),"")])]');
        assert.equal(parser.parse("(function() { hello.})();").toString(), '[Function("null",[],[PropAccess(Var("hello"),""),ERROR(),ERROR()])]');
        
        assert.equal(parser.parse("bla(start.);").toString(), '[Function("null",[],[PropAccess(Var("hello"),""),ERROR(),ERROR()])]');
