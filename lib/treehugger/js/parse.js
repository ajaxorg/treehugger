...
function getMeta(n) {
  return {line: n.lineno, pos: n.start};
}
...
exports.transform = function(n) {
  ...
  switch(tokens[n.type]) {
    ...
    case "function":
      var offset = meta.pos;
      var idMeta = getMeta(n);
      idMeta.pos += 9;
      return tree.cons("Function", [n.name ? tree.string(n.name, idMeta) : tree.string("", idMeta), tree.list(n.params.map(function(a) {
    ...
    case "var":
      return tree.cons("VarDecls", [tree.list(n.children.map(function(vardec) {
        var meta = getMeta(vardec);
        if(vardec.initializer) {
          return tree.cons("VarDeclInit", [tree.string(vardec.name, meta), transform(vardec.initializer)], meta);
        } else {
          return tree.cons("VarDecl", [tree.string(vardec.name, meta)], meta);
        }
      }))], meta);
    case "const":
      return tree.cons("ConstDecls", [tree.list(n.children.map(function(vardec) {
        var meta = getMeta(vardec);
        return tree.cons("ConstDeclInit", [tree.string(vardec.name, meta), transform(vardec.initializer)], meta);
      }))], meta);
    case "return":
    ...
    case "new":
      return tree.cons("New", [transform(n.children[0]), tree.list()], meta);
    case ".":
      var propMeta = getMeta(n.children[1]); //{line: n.children[1].lineno, pos: n.children[1].start};
      return tree.cons("PropAccess", [transform(n.children[0]), tree.string(n.children[1].value, propMeta)], meta);
    case "IDENTIFIER":
